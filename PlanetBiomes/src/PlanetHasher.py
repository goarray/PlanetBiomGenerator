...
def create_resource_id(folder_path: str, file_name: str) -> str:
    """
    Generates a full Starfield-style resource ID:
    res:<folder_crc>:<file_crc>:00000000
    """
    folder_crc = get_directory_crc(folder_path)
    file_crc = get_file_crc(file_name)
    return f"res:{folder_crc:08X}:{file_crc:08X}:00000000"


def create_simple_crc_id(name: str, suffix: str = "", index: int = 0) -> str:
    """
    ...
    return f"res:{crc:08X}:{crc:08X}:00000000"